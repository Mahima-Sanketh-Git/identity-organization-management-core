 ...
 * <p>
 * It is registered as an OSGi {@link Executor} service by {@code OrganizationManagementServiceComponent}
 * and is bound to a flow step by the name returned from {@link #getName()}. The organization name,
 * handle and attributes are read from {@link FlowOrganization}; the creating user is read from
 * {@link FlowUser}. Fields are routed into {@link FlowOrganization} by the flow engine based on the
 * {@code identifierType} of each input, so this executor never inspects raw user input itself.
 * <p>
...
public class OrganizationProvisioningExecutor implements Executor {
     ...
     * bundle's own data holder by {@code OrganizationManagementServiceComponent}.
     *
    ...
    private String createOrganization(FlowExecutionContext context, String parentOrganizationId)
        ...
        OrganizationManager organizationManager =
                OrganizationManagementDataHolder.getInstance().getOrganizationManager();
